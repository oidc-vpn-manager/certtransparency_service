...
class TestJSONFormatter:
    ...
    def test_format_with_process_and_thread(self):
        ...
        assert parsed['process_id'] == 12345
        assert parsed['thread_id'] == 67890
    ...
    def test_format_with_request_context(self):
        ...
        mock_request.headers.get.return_value = 'test-agent/1.0'

        # Mock Flask g with request ID
        mock_g = MagicMock()
        mock_g.request_id = 'req-123'

        with patch('app.utils.logging_config.request', mock_request), \
             patch('app.utils.logging_config.g', mock_g):
            result = formatter.format(record)
            parsed = json.loads(result)

            assert 'request_context' in parsed
            assert parsed['request_context']['method'] == 'POST'
            assert parsed['request_context']['path'] == '/api/test'
            assert parsed['request_context']['remote_addr'] == '192.168.1.1'
            assert parsed['request_context']['user_agent'] == 'test-agent/1.0'
            assert parsed['request_id'] == 'req-123'

    def test_format_with_request_context_no_request_id(self):
        """Test formatting with Flask request context but no request ID."""
        formatter = JSONFormatter()
        ...
        # Mock Flask g without request_id attribute
        mock_g = MagicMock()
        del mock_g.request_id  # Ensure attribute doesn't exist
        ...
        with patch('app.utils.logging_config.request', mock_request), \
            ...
            assert 'request_context' in parsed
            assert parsed['request_context']['method'] == 'GET'
            assert 'request_id' not in parsed

    def test_format_with_request_context_runtime_error(self):
        ...
        # Mock that accessing request in boolean context raises RuntimeError
        class MockRequest:
            def __bool__(self):
                raise RuntimeError("Working outside of request context")

        with patch('app.utils.logging_config.request', MockRequest()):
            result = formatter.format(record)
            parsed = json.loads(result)
...
class TestRequestIdMiddleware:
    ...
    def test_add_request_id_middleware(self):
        """Test request ID middleware adds request IDs."""
...
class TestConfigureSecurityLogging:
    ...
    def test_configure_security_logging_basic(self):
        ...
        with patch('app.utils.logging_config.setup_logging') as mock_setup:
            with patch('app.utils.logging_config.add_request_id_middleware') as mock_middleware:
                ...
                # Should call setup_logging and middleware
    ...
    def test_error_handler_500(self):
        ...
        with patch('app.utils.logging_config.setup_logging'):
            with patch('app.utils.logging_config.add_request_id_middleware'):
                configure_security_logging(app)
    ...
    def test_error_handler_404_static_path(self):
        """Test 404 error handler doesn't log static paths."""
        from flask import Flask
        ...
        with app.test_client() as client:
            for path in static_paths:
                ...
                # Should return 404 Not Found but not log
                assert response.status_code == 404